...
{
  private static final int TABLE_SIZE = 2039;
        ...
        synchronized (lockTable) {
          ...
          if (!bConflict) {
            ...
            if (bConvert.get(0)) {
              ...
              ((TransactionLockObject)lockTable.get(xLockObject)).setLockType(TransactionLockObject.LockType.LOCK_WRITE);
              ((TransactionLockObject)lockTable.get(dataLockObject)).setLockType(TransactionLockObject.LockType.LOCK_WRITE);
  ...
  // is set.
  private boolean lockConflict(DataLockObject newDataLock, BitSet bitset) throws DeadlockException, RedundantLockRequestException
  {
    // vect contains all the locks for a given data object
    Vector vect = lockTable.elements(newDataLock);
    int size = vect.size();

    // As soon as a lock that conflicts with the current lock request is found, return true
    for (int i = 0; i < size; i++)
    {
      DataLockObject existingDataLock = (DataLockObject)vect.elementAt(i);
      if (newDataLock.getXId() == existingDataLock.getXId())
      {
        // The transaction already has a lock on this data item which means that it is either
        // relocking it or is converting the lock
        if (newDataLock.getLockType() == TransactionLockObject.LockType.LOCK_READ)
        {
          ...
          // is redundant.
          throw new RedundantLockRequestException(newDataLock.getXId(), "redundant READ lock request");
        }
        else if (newDataLock.getLockType() == TransactionLockObject.LockType.LOCK_WRITE)
        {
      ...
      else if (newDataLock.getLockType() == TransactionLockObject.LockType.LOCK_READ)
      {
        if (existingDataLock.getLockType() == TransactionLockObject.LockType.LOCK_WRITE)
        {
          // Transaction is requesting a READ lock and some other transaction
          // already has a WRITE lock on it ==> conflict
          Trace.info("LM::lockConflict(" + newDataLock.getXId() + ", " + newDataLock.getDataName() + ") Want READ, someone has WRITE");
          return true;
        }
      }
      else if (newDataLock.getLockType() == TransactionLockObject.LockType.LOCK_WRITE)
      {
        ...
        Trace.info("LM::lockConflict(" + newDataLock.getXId() + ", " + newDataLock.getDataName() + ") Want WRITE, someone has READ or WRITE");
        return true;
      }
    }
